...
fn demonstrate_error_codes() {
    ...
    let errors = vec![
        ("Tool Timeout", AiCoreError::tool_timeout("Tool execution timed out")),
        ("Model Timeout", AiCoreError::model_timeout("Model inference timed out")),
        ("Network Error", AiCoreError::network("Connection failed")),
        ("Configuration Error", AiCoreError::config("Invalid configuration")),
        ("Capability Denied", AiCoreError::cap_denied("Insufficient permissions")),
        ("Circuit Breaker Open", AiCoreError::circuit_breaker_open("Service temporarily disabled")),
    ];
...
async fn demonstrate_retry_policies() -> Result<(), Box<dyn std::error::Error>> {
...
async fn demonstrate_deterministic_retries() -> Result<(), Box<dyn std::error::Error>> {
...
async fn demonstrate_audit_logging() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let errors = vec![
        (AiCoreError::tool_timeout("Tool execution timed out"), "tool_timeout_test"),
    ...
    if let Some(logger) = get_global_error_audit_logger() {
        ...
        for (i, entry) in entries.iter().enumerate() {
            println!("    Entry {}: Code {}, {} retry attempts", 
                    i + 1, entry.error_code, entry.retry_attempts.len());
        }
    }
...
async fn simulate_failing_operation(attempt: u32, should_succeed_after: u32) -> Result<String, AiCoreError> {
    if attempt < should_succeed_after {
        Err(AiCoreError::network(&format!("Simulated failure on attempt {}", attempt)))
    } else {
...
async fn demonstrate_real_world_retry() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = retry_with_backoff(policy, || {
        ...
        Box::pin(async move {
            simulate_failing_operation(unsafe { ATTEMPT }, 3).await
        })
    }).await;
