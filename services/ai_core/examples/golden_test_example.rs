...
impl MockAiCoreClient {
    ...
    async fn simulate_ai_response(&self, prompt: &TestPrompt) -> MockResponse {
        ...
        let response_text = match prompt.id.as_str() {
            ...
            "file_operation" => "File 'test.txt' created successfully with content 'Hello World'.".to_string(),
            "search_query" => "Found information about artificial intelligence in the knowledge base.".to_string(),
            "complex_reasoning" => "Supervised learning uses labeled data to train models, while unsupervised learning finds patterns in unlabeled data. Examples include classification (supervised) and clustering (unsupervised).".to_string(),
            ...
            "multimodal_request" => "I can see an image, but I cannot process it in this demonstration.".to_string(),
...
fn create_sample_config() -> GoldenTestConfig {
    ...
    let prompts = vec![
        ("simple_question", "The capital of France is Paris.", 25, vec![]),
        ("math_problem", "15 + 27 = 42", 15, vec![]),
        ("code_generation", "def factorial(n):\n    if n <= 1:\n        return 1\n    return n * factorial(n-1)", 120, vec![]),
        ("file_operation", "File 'test.txt' created successfully with content 'Hello World'.", 45, vec!["create_file"]),
        ("search_query", "Found information about artificial intelligence in the knowledge base.", 80, vec!["search_files"]),
        ("complex_reasoning", "Supervised learning uses labeled data to train models, while unsupervised learning finds patterns in unlabeled data. Examples include classification (supervised) and clustering (unsupervised).", 200, vec![]),
        ("system_command", "Current system time: 2024-01-15 10:30:45 UTC", 30, vec!["get_system_time"]),
        ("memory_operation", "Remembered: Your favorite color is blue.", 35, vec!["mem.put"]),
        ("error_handling", "Error: Division by zero is undefined in mathematics.", 40, vec![]),
        ("multimodal_request", "I can see an image, but I cannot process it in this demonstration.", 90, vec![]),
    ];
    ...
    for (id, content, tokens, tool_calls) in prompts {
        ...
        expected_outputs.insert(id.to_string(), ExpectedOutput {
        ...
        prompts: vec![
            TestPrompt {
                ...
                expected_response: PromptExpectation {
                    min_length: 10,
                    max_length: 100,
                    keywords: vec!["Paris".to_string(), "France".to_string(), "capital".to_string()],
            ...
            TestPrompt {
                ...
                text: "Write a simple Python function to calculate the factorial of a number.".to_string(),
                expected_response: PromptExpectation {
                    min_length: 50,
                    max_length: 500,
                    keywords: vec!["def".to_string(), "factorial".to_string(), "python".to_string(), "function".to_string()],
            ...
            TestPrompt {
                ...
                text: "Create a new file called 'test.txt' with the content 'Hello World'.".to_string(),
                expected_response: PromptExpectation {
                    min_length: 20,
                    max_length: 200,
                    keywords: vec!["file".to_string(), "created".to_string(), "test.txt".to_string()],
            ...
            TestPrompt {
                ...
                expected_response: PromptExpectation {
                    min_length: 30,
                    max_length: 300,
                    keywords: vec!["artificial".to_string(), "intelligence".to_string(), "AI".to_string()],
                    response_type: ResponseType::ToolCall,
                },
            ...
            TestPrompt {
                ...
                text: "Explain the difference between supervised and unsupervised machine learning, and provide examples of each.".to_string(),
                expected_response: PromptExpectation {
                    min_length: 100,
                    max_length: 800,
                    keywords: vec!["supervised".to_string(), "unsupervised".to_string(), "machine learning".to_string(), "examples".to_string()],
                    response_type: ResponseType::Text,
                },
                expected_tool_calls: vec![],
            },
            TestPrompt {
                ...
                expected_response: PromptExpectation {
                    min_length: 10,
                    max_length: 100,
                    keywords: vec!["time".to_string(), "current".to_string(), "system".to_string()],
                    response_type: ResponseType::ToolCall,
                },
            ...
            TestPrompt {
                ...
                expected_response: PromptExpectation {
                    min_length: 15,
                    max_length: 100,
                    keywords: vec!["remembered".to_string(), "favorite".to_string(), "color".to_string(), "blue".to_string()],
                    response_type: ResponseType::ToolCall,
                },
            ...
            TestPrompt {
                ...
                expected_response: PromptExpectation {
                    min_length: 20,
                    max_length: 150,
                    keywords: vec!["error".to_string(), "division".to_string(), "zero".to_string(), "undefined".to_string()],
                    response_type: ResponseType::Text,
                },
                expected_tool_calls: vec![],
            },
            TestPrompt {
                ...
                expected_response: PromptExpectation {
                    min_length: 30,
                    max_length: 400,
                    keywords: vec!["image".to_string(), "see".to_string(), "describe".to_string()],
                    response_type: ResponseType::Text,
                },
                expected_tool_calls: vec![],
            },
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Created golden test configuration with {} prompts", config.prompts.len());
    ...
    for result in &report.results {
        ...
        println!("  {} {}: {} tokens, {}ms", 
                status, 
                result.prompt_id, 
                result.actual_token_count,
                result.actual_processing_time_ms);
    ...
    if let Some(expected) = rebaseline_config.expected_outputs.get_mut("simple_question") {
    ...
    let rebaseline_result = rebaseline_client.run_test(&rebaseline_client.config.prompts[0]).await;
    
    println!("  Test: {}", rebaseline_result.prompt_id);
    println!("  Expected hash: {}", rebaseline_result.expected_response_hash);
    println!("  Actual hash:   {}", rebaseline_result.actual_response_hash);
