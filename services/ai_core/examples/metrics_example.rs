...
fn display_metrics(metrics: &AiCoreMetrics) {
    ...
    println!("  Memory usage:       {:.1} MB", metrics.model_memory_usage_mb);
    ...
    println!("  Avg duration:       {:.1} seconds", metrics.session_duration_avg_secs);
}
...
async fn test_ngfs_storage(metrics: &AiCoreMetrics) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Retrieved metrics: {} requests, {} tokens", 
