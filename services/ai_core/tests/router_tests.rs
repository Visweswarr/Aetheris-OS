...
use aetheris_ai_core::router::{
    ConversationTurn, Intent, PromptRouter, RenderContext, TemplateValidator, TemplateVariables,
};
...
async fn test_custom_variables_rendering() {
    ...
    assert_eq!(
        context.variables.custom_vars.get("custom_var"),
        Some(&"custom_value".to_string())
    );
    assert_eq!(
        context.variables.custom_vars.get("another_var"),
...
async fn test_clean_unreplaced_placeholders() {
    ...
    assert_eq!(
        cleaned,
        "Hello {{user_message}} [MISSING_VARIABLE] World [MISSING_VARIABLE]"
    );
}
...
async fn test_router_reload_templates() {
    ...
    fs::write(&new_template_path, "Custom template: {{user_message}}")
        .await
        .unwrap();
...
async fn test_intent_from_string_conversion() {
    ...
    assert_eq!(
        Intent::from("custom_intent"),
        Intent::Custom("custom_intent".to_string())
    );
...
async fn test_router_has_template() {
    ...
    assert!(
        !router
            .has_template(&Intent::Custom("nonexistent".to_string()))
            .await
    );
}
...
async fn test_complex_template_rendering() {
    ...
    request.conversation_history = Some(vec![aetheris_ai_core::ipc::ConversationTurn {
        role: "user".to_string(),
    ...
    assert_eq!(
        context.variables.custom_vars.get("data_type"),
        Some(&"sales".to_string())
    );
    assert_eq!(
        context.variables.custom_vars.get("timeframe"),
        Some(&"Q1 2024".to_string())
    );
}
