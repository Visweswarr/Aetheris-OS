...
async fn test_retry_context_should_retry() {
    ...
    let context = RetryContext::new(policy.clone());
...
async fn test_retry_with_backoff_success() {
    ...
    let result: aetheris_ai_core::error::Result<&str> = retry_with_backoff(policy, || {
        attempt_count += 1;
        Box::pin(async move {
            if attempt_count == 1 {
...
async fn test_retry_with_backoff_exhaustion() {
    ...
    let result: aetheris_ai_core::error::Result<&str> = retry_with_backoff(policy, || {
        attempt_count += 1;
        Box::pin(async move { Err(AiCoreError::network("Persistent failure")) })
    })
    .await;
...
async fn test_retry_with_backoff_non_retryable_error() {
    ...
    let result: aetheris_ai_core::error::Result<&str> = retry_with_backoff(policy, || {
        attempt_count += 1;
        Box::pin(async move { Err(AiCoreError::config("Configuration error")) })
    })
    .await;
