...
use aetheris_ai_core::cap::{CapTokenManager, PolicyEnforcer, CapabilityPolicy, PolicyEnforcementResult};
...
async fn test_capability_check_denied_wrong_resource() {
    ...
    let result = manager.check_capability(&token, "wrong_resource", "generate_response").await;
    assert!(result.is_ok());
...
async fn test_capability_check_denied_wrong_action() {
    ...
    let result = manager.check_capability(&token, "ai_core", "wrong_action").await;
    assert!(result.is_ok());
...
async fn test_capability_check_with_deny_error() {
    ...
    // This should succeed
    let result = manager.check_capability_with_deny(&token, "ai_core", "generate_response").await;
    assert!(result.is_ok());
    
    // This should fail with CapDenied error
    let result = manager.check_capability_with_deny(&token, "wrong_resource", "generate_response").await;
...
async fn test_policy_enforcement_direct() {
    ...
    let result = manager.enforce_capability_policy("ai:chat", "ai_core", "generate_response", "aetheris-system").await;
    assert!(result.is_ok());
...
async fn test_policy_enforcement_denied() {
    ...
    let result = manager.enforce_capability_policy("unknown:capability", "ai_core", "generate_response", "aetheris-system").await;
    assert!(result.is_ok());
...
async fn test_token_validation_expired() {
    ...
    token.expires_at = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() - 3600; // Expired
...
async fn test_policy_enforcer_rule_matching() {
    ...
    let result = enforcer.enforce_policy("ai:chat", "ai_core", "generate_response", "aetheris-system").await;
    assert!(result.is_ok());
...
async fn test_policy_enforcer_rule_matching_denied() {
    ...
    let result = enforcer.enforce_policy("unknown:capability", "ai_core", "generate_response", "aetheris-system").await;
    assert!(result.is_ok());
...
async fn test_policy_enforcer_issuer_validation() {
    ...
    // Test unknown issuer
    let result = enforcer.enforce_policy("ai:chat", "ai_core", "generate_response", "unknown-issuer").await;
    assert!(result.is_ok());
...
async fn test_policy_enforcer_deny_by_default() {
    ...
    let result = enforcer.enforce_policy("ai:chat", "unknown_resource", "unknown_action", "aetheris-system").await;
    assert!(result.is_ok());
...
async fn test_capability_check_caching() {
    ...
    // First check
    let result1 = manager.check_capability(&token, "ai_core", "generate_response").await;
    assert!(result1.is_ok());
    
    // Second check (should use cache)
    let result2 = manager.check_capability(&token, "ai_core", "generate_response").await;
...
async fn test_capability_check_concurrent() {
    ...
    for _ in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            manager_clone.check_capability(&token_clone, "ai_core", "generate_response").await
        });
        handles.push(handle);
    }
...
async fn test_capability_check_memory_usage() {
    ...
    for i in 0..1000 {
        ...
        let result = manager.check_capability(&token_clone, "ai_core", "generate_response").await;
        assert!(result.is_ok());
    }
...
async fn test_policy_enforcer_edge_cases() {
    ...
    for (capability, resource, action, issuer) in edge_cases {
        let result = enforcer.enforce_policy(capability, resource, action, issuer).await;
        // Should handle edge cases gracefully
        assert!(result.is_ok());
    }
}
