...
use aetheris_ai_core::intents::{
    OrderedFloat, PlatformAdapter, SystemActionContext, SystemIntent, SystemIntentManager,
};
...
async fn create_test_intent_manager() -> SystemIntentManager {
    ...
    let cap_token_manager = Arc::new(
        CapTokenManager::new(&temp_dir.path().join("cap.toml"))
            .await
            .unwrap(),
    );
    let adapter = Arc::new(PlatformAdapter::Mock(
        aetheris_ai_core::intents::mock::MockSystemAdapter::new(),
    ));
...
async fn test_intent_parsing_settings() {
    ...
    assert_eq!(
        intent,
        SystemIntent::OpenSettingsCategory("display".to_string())
    );

    let intent = manager.parse_intent("Open sound settings").unwrap();
    assert_eq!(
        intent,
        SystemIntent::OpenSettingsCategory("sound".to_string())
    );

    let intent = manager.parse_intent("Open network settings").unwrap();
    assert_eq!(
        intent,
        SystemIntent::OpenSettingsCategory("network".to_string())
    );
}
...
async fn test_intent_parsing_volume() {
    ...
    assert_eq!(intent, SystemIntent::AdjustVolume(OrderedFloat(0.5)));

    let intent = manager.parse_intent("Set volume to 75%").unwrap();
    assert_eq!(intent, SystemIntent::AdjustVolume(OrderedFloat(0.75)));

    // Test volume with decimal
    let intent = manager.parse_intent("Set volume to 0.8").unwrap();
    assert_eq!(intent, SystemIntent::AdjustVolume(OrderedFloat(0.8)));

    // Test volume adjustments
    let intent = manager.parse_intent("Turn volume up").unwrap();
    assert_eq!(intent, SystemIntent::AdjustVolume(OrderedFloat(0.1)));
...
async fn test_intent_parsing_brightness() {
    ...
    assert_eq!(intent, SystemIntent::SetBrightness(OrderedFloat(0.8)));

    let intent = manager.parse_intent("Set brightness to 100%").unwrap();
    assert_eq!(intent, SystemIntent::SetBrightness(OrderedFloat(1.0)));

    // Test brightness with decimal
    let intent = manager.parse_intent("Set brightness to 0.6").unwrap();
    assert_eq!(intent, SystemIntent::SetBrightness(OrderedFloat(0.6)));

    // Test brightness adjustments
    let intent = manager.parse_intent("Turn brightness up").unwrap();
    assert_eq!(intent, SystemIntent::SetBrightness(OrderedFloat(0.1)));
    ...
    match intent {
        SystemIntent::Custom(name, params) => {
            ...
            assert_eq!(
                ...
                Value::String("Some unknown command".to_string())
            );
        }
...
async fn test_intent_execution_volume() {
    ...
    let intent = SystemIntent::AdjustVolume(OrderedFloat(0.75));
    let result = manager.execute_intent(&intent, &context).await.unwrap();
...
async fn test_intent_execution_brightness() {
    ...
    let intent = SystemIntent::SetBrightness(OrderedFloat(0.8));
    let result = manager.execute_intent(&intent, &context).await.unwrap();
...
async fn test_required_capabilities() {
    ...
    let caps = manager.get_required_capabilities(&SystemIntent::AdjustVolume(OrderedFloat(0.0)));
    assert!(caps.contains(&"hardware.audio.modify".to_string()));

    // Test brightness intent capabilities
    let caps = manager.get_required_capabilities(&SystemIntent::SetBrightness(OrderedFloat(0.0)));
...
async fn test_parameterized_intent_capabilities() {
    ...
    let caps = manager
        .get_required_capabilities(&SystemIntent::OpenSettingsCategory("display".to_string()));
    assert!(caps.contains(&"settings.read".to_string()));
    ...
    let caps = manager.get_required_capabilities(&SystemIntent::AdjustVolume(OrderedFloat(0.5)));
    assert!(caps.contains(&"hardware.audio.modify".to_string()));

    let caps = manager.get_required_capabilities(&SystemIntent::SetBrightness(OrderedFloat(0.8)));
    assert!(caps.contains(&"hardware.display.modify".to_string()));
}
...
async fn test_intent_execution_context_metadata() {
    ...
    context
        .metadata
        .insert("test_number".to_string(), Value::Number(42.into()));
...
async fn test_intent_execution_error_handling() {
    ...
    let result = manager
        .execute_intent(&unsupported_intent, &context)
        .await
        .unwrap();
    assert!(result.success);
}
...
async fn test_intent_parsing_complex_phrases() {
    ...
    let intent = manager
        .parse_intent("Please open the system settings")
        .unwrap();
    assert_eq!(intent, SystemIntent::OpenSettings);

    let intent = manager
        .parse_intent("Can you turn on Bluetooth for me")
        .unwrap();
    assert_eq!(intent, SystemIntent::ToggleBluetooth(true));
    ...
    let intent = manager
        .parse_intent("Could you please lock the screen")
        .unwrap();
    assert_eq!(intent, SystemIntent::LockScreen);
}
