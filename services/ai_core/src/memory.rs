...
pub struct MemoryStore {
    ...
    redaction_patterns: Arc<RwLock<Vec<(String, String)>>>,
    stats: Arc<RwLock<MemoryStats>>,
}

impl MemoryStore {
    pub fn new(config: MemoryConfig) -> Result<Self> {
        let redaction_patterns: Vec<(String, String)> = config.redaction_policies.iter()
            .map(|p| (p.name.clone(), p.pattern.clone())).collect();
        Ok(Self {
            config,
            ...
            stats: Arc::new(RwLock::new(MemoryStats {
                ...
                total_access_count: 0, storage_size_bytes: 0, last_cleanup: 0, last_ngfs_snapshot: None,
            })),
        })
    }
    ...
    pub async fn put(&self, key: String, value: String, tags: Vec<String>, ttl_seconds: Option<u64>, user_id: String, session_id: Option<String>) -> Result<()> {
        ...
        let entry = MemoryEntry {
            key: key.clone(), value: redacted_value, tags: tags.clone(),
            created_at: now, expires_at, access_count: 0, last_accessed: now,
            user_id, session_id, redacted: was_redacted,
        };
    ...
    pub async fn query(&self, tags: &[String]) -> Result<Vec<MemoryEntry>> {
        ...
        for tag in tags {
            if let Some(keys) = tag_index.get(tag) {
                if matching_keys.is_empty() { matching_keys = keys.clone(); }
                else { matching_keys = matching_keys.intersection(keys).cloned().collect(); }
            } else { return Ok(Vec::new()); }
        }

        let mut results: Vec<_> = matching_keys.iter()
    ...
    pub async fn delete(&self, key: &str) -> Result<bool> {
        ...
        if removed { self.update_tag_index_remove(key).await?; self.update_stats().await?; }
        Ok(removed)
    }
    ...
    async fn update_tag_index(&self, key: &str, tags: &[String]) -> Result<()> {
        ...
        for tag in tags {
            index.entry(tag.clone()).or_insert_with(HashSet::new).insert(key.to_string());
        }
        Ok(())
    }
    ...
    async fn update_tag_index_remove(&self, key: &str) -> Result<()> {
        let mut index = self.tag_index.write().await;
        for keys in index.values_mut() { keys.remove(key); }
    ...
    async fn update_stats(&self) -> Result<()> {
        ...
        stats.active_entries = entries.values().filter(|e| e.expires_at.is_none_or(|exp| now <= exp)).count();
        ...
        stats.storage_size_bytes = entries.values().map(|e| (e.key.len() + e.value.len()) as u64).sum();
        Ok(())
    }
                ...
                if let Ok(loaded) = serde_cbor::from_slice::<HashMap<String, MemoryEntry>>(&data) {
                    ...
                    for (key, entry) in loaded.iter() {
                        for tag in &entry.tags {
                            index.entry(tag.clone()).or_insert_with(HashSet::new).insert(key.clone());
                        }
                    }
    ...
    pub async fn handle_put_tool(&self, request: &ToolCallRequest) -> Result<ToolCallResponse> {
        let args: HashMap<String, serde_json::Value> = serde_json::from_str(&request.arguments).unwrap_or_default();
        ...
        let user_id = request.user_id.clone().unwrap_or_else(|| "unknown".to_string());
        
        self.put(key.clone(), value, tags, ttl, user_id, request.session_id.clone()).await?;
        
        Ok(ToolCallResponse {
            result: format!("Stored: {}", key), success: true, error_message: String::new(),
            metrics: None, cbor_payload: None, call_id: request.call_id.clone(),
            tool_version: "1.0.0".to_string(), metadata: None, warnings: Vec::new(), exit_code: 0,
        })
    }
    ...
    pub async fn handle_get_tool(&self, request: &ToolCallRequest) -> Result<ToolCallResponse> {
        ...
        let result = entry.map(|e| serde_json::to_string(&e).unwrap_or_default()).unwrap_or_else(|| "null".to_string());
        
        Ok(ToolCallResponse {
            result, success: true, error_message: String::new(),
            metrics: None, cbor_payload: None, call_id: request.call_id.clone(),
            tool_version: "1.0.0".to_string(), metadata: None, warnings: Vec::new(), exit_code: 0,
        })
    }
    ...
    pub async fn handle_delete_tool(&self, request: &ToolCallRequest) -> Result<ToolCallResponse> {
        ...
        Ok(ToolCallResponse {
            result: format!("Deleted: {}", deleted), success: true, error_message: String::new(),
            metrics: None, cbor_payload: None, call_id: request.call_id.clone(),
            tool_version: "1.0.0".to_string(), metadata: None, warnings: Vec::new(), exit_code: 0,
        })
    }
...
pub fn default_redaction_policies() -> Vec<RedactionPolicy> {
    vec![
        RedactionPolicy { name: "password".to_string(), pattern: r"(?i)(password|passwd|pwd)\s*[:=]\s*\S+".to_string(), replacement: "[REDACTED]".to_string(), description: None },
        RedactionPolicy { name: "api_key".to_string(), pattern: r"(?i)(api[_-]?key|apikey)\s*[:=]\s*\S+".to_string(), replacement: "[REDACTED]".to_string(), description: None },
        RedactionPolicy { name: "token".to_string(), pattern: r"(?i)(token|bearer)\s*[:=]\s*\S+".to_string(), replacement: "[REDACTED]".to_string(), description: None },
    ]
}

pub fn default_memory_config(base_path: PathBuf) -> MemoryConfig {
    MemoryConfig {
        base_path, max_entries: 10000, default_ttl_seconds: 86400, cleanup_interval_seconds: 3600,
        enable_redaction: true, redaction_policies: default_redaction_policies(),
        enable_ngfs: false, ngfs_snapshot_interval_seconds: 86400,
    }
}
