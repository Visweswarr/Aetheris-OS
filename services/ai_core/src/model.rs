...
impl ModelBackendImpl {
    pub async fn generate_response(&self, request: &ModelRequest) -> Result<ModelResponse> {
        match self {
            ModelBackendImpl::Mock(m) => m.generate_response(request).await,
        }
    }
    pub fn get_info(&self) -> ModelInfo {
        match self {
            ModelBackendImpl::Mock(m) => m.get_info(),
        }
    }
    pub fn is_ready(&self) -> bool {
        match self {
            ModelBackendImpl::Mock(m) => m.is_ready(),
        }
    }
}
...
impl ModelManager {
    ...
    pub async fn generate_response(&self, request: &ChatRequest) -> Result<ModelResponse> {
        ...
        let model = models
            .get(&model_name)
            .or_else(|| models.get(&self.default_model))
    ...
    async fn load_models(&self) -> Result<()> {
        ...
        models.insert(
            "mock".to_string(),
            Arc::new(ModelBackendImpl::Mock(MockModel::new("mock".to_string()))),
        );
        // Loaded mock model
        Ok(())
    }
