...
/// Page size type alias
pub type PageSize = usize;

/// Memory flags for page mappings
...
pub fn init_mm() {
    ...
    kprintln!("[MM] Bootstrap allocator active — early heap allocations safe");
    ...
    // Test paging functionality only in diagnostic builds; normal boot should
    // initialize MM and continue to the scheduler/main loop.
...
pub use manager::{
    ...
    PageFault, FaultReason, PressureStatus, MemoryManagerStats,
    MEMORY_MANAGER, init_memory_manager,
    // Capability-enforced memory access (Requirement 1.4)
    handle_capability_page_fault, validate_and_enforce_access,
    register_termination_callback, ProcessTerminationCallback,
    PageFaultResolution, AccessType,
};
...
/// Enable the stage-0 allocator used before the full kernel heap is online.
pub fn init_bootstrap_allocator() {
    bootstrap_alloc::activate_bootstrap();
}
...
unsafe impl GlobalAlloc for KernelAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        // During early boot, use the bootstrap allocator (static bump buffer).
        // Once mm::alloc::init() completes, this path is bypassed.
        if crate::mm::bootstrap_alloc::is_active() {
            return crate::mm::bootstrap_alloc::bootstrap_alloc(layout.size(), layout.align());
        }

        // Normal path: use the real kmalloc allocators.
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        // Bootstrap allocations are permanent — never freed.
        // Check if the pointer falls within the bootstrap heap range.
        if crate::mm::bootstrap_alloc::is_bootstrap_ptr(ptr) {
            return; // no-op for bootstrap allocations
        }
