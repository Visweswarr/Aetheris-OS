...
pub fn init() {
    ...
    // Map kernel sections
    map_kernel_sections().expect("Failed to map kernel sections");
    
    // Create kernel stack with guard pages
    create_kernel_stacks_with_guards().expect("Failed to create kernel stacks with guard pages");
    ...
    
    {
        ...
        if let Some(ref mut state) = *paging_state {
            ...
            match map_result {
                ...
                Err(err) => {
                    klog!(ERROR, "[PAGING] Failed to map page: {:?}", err);
