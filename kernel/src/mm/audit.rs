...
impl AuditStats {
    /// Create new audit statistics with default values (const-friendly)
    pub const fn new() -> Self {
        Self {
            total_audits: 0,
            total_pages_audited: 0,
            total_suspicious_pages: 0,
            total_wx_violations: 0,
...
impl PageTableWalker {
    ...
    pub fn walk_and_audit(&mut self) -> Result<AuditResult, String> {
        let mut page_table = self.page_table.take()
        ...
        for (start, end, region_name) in AUDIT_REGIONS {
            ...
            if let Err(e) = self.audit_region(&mut page_table, *start, *end, region_name) {
                result = Err(e);
                break;
            }
        }
    ...
    fn audit_region(&mut self, page_table: &mut OffsetPageTable,
        ...
        while current_page < end_page && self.continue_walking {
            ...
            if let TranslateResult::Mapped { frame, flags, .. } = page_table.translate(current_page.start_address()) {
                if let MappedFrame::Size4KiB(phys_frame) = frame {
                    self.audit_page(
    ...
    fn audit_page(&mut self, vaddr: u64, paddr: u64, flags: PageFlags, region: &str) {
        ...
        if !issues.is_empty() {
            ...
            let suspicious_page = SuspiciousPage::new(vaddr, paddr, flags, reason.clone(), region.to_string());
...
impl AuditManager {
    /// Create new audit manager (const-friendly)
    pub const fn new() -> Self {
        Self {
...
static AUDIT_STATS: spin::Mutex<AuditStats> = spin::Mutex::new(AuditStats::new());
