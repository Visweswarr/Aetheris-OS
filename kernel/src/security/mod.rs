...
// Re-export key types and functions
pub use cap::{
    CapToken, CapabilityStore, scope,
    grant_capability, check_capability, revoke_capability,
    revoke_capability_globally, cleanup_expired_capabilities,
    get_capability_stats, print_all_capabilities, test_capability_storage,
};

/// Lightweight wrapper around `cap::validate_cap` that pulls the global capability store.
pub fn validate_cap(tok: &CapToken, dst: u64, now_ms: u64) -> bool {
    cap::validate_cap_simple(tok, dst, now_ms)
}
pub use cap_v2::*;

/// Scope V2 constants
pub mod scope_v2 {
    pub const SEND: u64 = 0x1;
    pub const RECV: u64 = 0x2;
    pub const CALL: u64 = 0x4;
    pub const ADMIN: u64 = 0x8;
}
