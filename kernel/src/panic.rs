...
unsafe fn read_general_purpose_registers() -> GeneralPurposeRegisters {
    ...
    // We need to be careful here - some registers might be modified by the compiler
    // We'll capture them in a way that doesn't interfere with the current execution
    asm!(
        // Save current registers to memory
        "mov {rax_out}, rax",
        "mov {rbx_out}, rbx", 
        "mov {rcx_out}, rcx",
        "mov {rdx_out}, rdx",
        ...
        "mov {r14_out}, r14",
        "mov {r15_out}, r15",
        rax_out = out(reg) gpr.rax,
        rbx_out = out(reg) gpr.rbx,
        rcx_out = out(reg) gpr.rcx,
        rdx_out = out(reg) gpr.rdx,
        rsi_out = out(reg) gpr.rsi,
        rdi_out = out(reg) gpr.rdi,
        rbp_out = out(reg) gpr.rbp,
        rsp_out = out(reg) gpr.rsp,
        r8_out = out(reg) gpr.r8,
        r9_out = out(reg) gpr.r9,
        r10_out = out(reg) gpr.r10,
        r11_out = out(reg) gpr.r11,
        r12_out = out(reg) gpr.r12,
        r13_out = out(reg) gpr.r13,
        r14_out = out(reg) gpr.r14,
        r15_out = out(reg) gpr.r15,
...
unsafe fn read_control_registers() -> ControlRegisters {
    ...
    asm!(
        ...
        "mov {cr3_out}, cr3",
        "mov {cr4_out}, cr4",
        cr0_out = out(reg) ctrl.cr0,
        cr2_out = out(reg) ctrl.cr2,
        cr3_out = out(reg) ctrl.cr3,
        cr4_out = out(reg) ctrl.cr4,
        options(preserves_flags, readonly, nostack)
    );
    ...
    asm!(
        ...
        "mov {fs_out:x}, fs",
        "mov {gs_out:x}, gs",
        cs_out = out(reg) ctrl.cs,
        ss_out = out(reg) ctrl.ss,
        ds_out = out(reg) ctrl.ds,
        es_out = out(reg) ctrl.es,
        fs_out = out(reg) ctrl.fs,
        gs_out = out(reg) ctrl.gs,
        options(preserves_flags, readonly, nostack)
    );
    ...
    asm!(
        "pushfq",
        "pop {rflags_out}",
        rflags_out = out(reg) ctrl.rflags,
        options(preserves_flags)
    );
    
    // Get current instruction pointer (approximate - will be the next instruction)
    asm!(
        "lea {rip_out}, [rip]",
        rip_out = out(reg) ctrl.rip,
        options(preserves_flags, readonly, nostack)
    );
...
fn write_minidump_to_buffer(cpu_state: &CpuState, panic_info: &PanicInfo) -> Result<(), &'static str> {
    ...
    // Write the minidump
    write_minidump(&cpu_regs, panic_info, &stack_trace)
}
