...
/// Alias for LogLevel for compatibility
...
pub fn init_levels() { 
    ...
    crate::kprintln!("[LOG] Enhanced logging system initialized with rate limiting");
}
...
pub fn set_level(level: LogLevel) {
    ...
    log_with_rate_limit(LogLevel::INFO, tags::BOOT, &crate::kformat!("Log level set to {}", level.as_str()));
}
...
pub fn print_rate_limit_stats() {
    ...
    crate::kprintln!("");
    crate::kprintln!("=== RATE LIMITING STATISTICS ===");
    crate::kprintln!("Active tracked messages: {}", active_entries);
    crate::kprintln!("Total suppressions: {}", total_suppressions);
    crate::kprintln!("Rate limit threshold: {} messages per {}ms", RATE_LIMIT_THRESHOLD, RATE_LIMIT_WINDOW_MS);
    crate::kprintln!("=== END RATE LIMITING STATISTICS ===");
    crate::kprintln!("");
}
...
pub fn test_rate_limiting() {
    crate::kprintln!("");
    crate::kprintln!("=== RATE LIMITING TEST ===");
    ...
    crate::kprintln!("=== RATE LIMITING TEST COMPLETE ===");
    crate::kprintln!("");
}
...
macro_rules! klog {
    ...
    // Compatibility rules for Level::XYZ
    (Level::TRACE, [$tag:expr], $($t:tt)*) => { klog!(TRACE, [$tag], $($t)*) };
    (Level::DEBUG, [$tag:expr], $($t:tt)*) => { klog!(DEBUG, [$tag], $($t)*) };
    (Level::INFO, [$tag:expr], $($t:tt)*) => { klog!(INFO, [$tag], $($t)*) };
    (Level::WARN, [$tag:expr], $($t:tt)*) => { klog!(WARN, [$tag], $($t)*) };
    (Level::ERROR, [$tag:expr], $($t:tt)*) => { klog!(ERROR, [$tag], $($t)*) };
}