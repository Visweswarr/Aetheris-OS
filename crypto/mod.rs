...
//! 
//! This module provides cryptographic primitives and algorithms for Polymera OS,
//! including post-quantum cryptography implementations.
//! 
//! # Features
//! 
//! - **Post-Quantum Cryptography**: Kyber KEM and Dilithium signatures
//! - **Memory Safety**: Zeroization and automatic cleanup
...
pub mod utils {
    ...
    pub fn random_bytes(length: usize) -> Result<Vec<u8>, LibOqsError> {
        let mut buffer = vec![0u8; length];
        let result = unsafe {
            liboqs::bindings::liboqs_random_bytes(buffer.as_mut_ptr(), length)
        };
        
        if result == liboqs::bindings::LIBOQS_SUCCESS {
            Ok(buffer)
        } else {
    ...
    pub fn constant_time_compare(a: &[u8], b: &[u8]) -> bool {
        ...
        let result = unsafe {
            liboqs::bindings::liboqs_constant_time_compare(a.as_ptr(), b.as_ptr(), a.len())
        };
        
        result != 0
    }

    /// Zeroize a byte array (secure memory clearing)
    pub fn zeroize(data: &mut [u8]) {
        unsafe {
            liboqs::bindings::liboqs_zeroize(data.as_mut_ptr(), data.len());
        }
    }
    ...
    pub fn provides_forward_secrecy(algorithm: &Algorithm) -> bool {
