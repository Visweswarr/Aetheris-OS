...
impl Dilithium {
    ...
    pub fn sign(
        ...
        if message.is_empty() {
            return Err(LibOqsError::InvalidParameter("Message cannot be empty".to_string()));
        }
    ...
    pub fn verify(
        ...
        if signature.is_empty() {
            return Err(LibOqsError::InvalidParameter("Signature cannot be empty".to_string()));
        }
        ...
        match result {
            LIBOQS_SUCCESS => Ok(true),
            LIBOQS_ERROR_VERIFICATION => Ok(false),
            _ => Err(LibOqsError::from(result)),
        }
    }
...
mod tests {
    ...
    fn test_dilithium_empty_message() {
        ...
        let empty_message = b"";
        assert!(Dilithium::sign(&secret_key, empty_message).is_err());
        assert!(Dilithium::verify(&public_key, empty_message, &DilithiumSignature::new(
            params,
            vec![0u8; params.signature_length()]
        ).unwrap()).is_err());
    }

    #[test]
    fn test_dilithium_empty_signature() {
        let params = DilithiumParameterSet::Dilithium2;
        let (public_key, _) = Dilithium::generate_keypair(params).unwrap();

        let message = b"Test message";
        let empty_signature = DilithiumSignature::new(params, vec![]).unwrap();
        assert!(Dilithium::verify(&public_key, message, &empty_signature).is_err());
    }
}
