 ...
 * 
 * This file provides the Linux-specific FUSE implementation using libfuse3.
 * It implements the aeth_fuse interface for Linux systems.
 */
...
static int fuse_readdir_callback(const char* path, void* buf, fuse_fill_dir_t filler,
    ...
    int wrapper_filler(void* buf, const char* name, const struct stat* stbuf, off_t off) {
        return filler(buf, name, stbuf, off, (enum fuse_fill_dir_flags)0);
    };
...
static int fuse_init_callback(struct fuse_conn_info* conn, struct fuse_config* cfg) {
    (void)conn; // Unused
    ...
    return fuse_state.ops->init(&ctx);
}
...
int aeth_fuse_available(void) {
    // Check if libfuse3 is available at compile time
    return 1;
}
